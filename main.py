...
def create_text_tile(combined_renewable_perc, output_folder, timestamp):
    ...
    draw.text(
        (10, y_position + 50), date_time_text[1], fill="black", font=timestamp_font
    )
...
def create_colored_tile(wind_perc, solar_perc, output_folder, timestamp):
    ...
    if combined_renewable_perc <= 30:
        ...
        color = tuple(
            int(green_color[i] * factor + grey_color[i] * (1 - factor))
            for i in range(3)
        )
...
def generate_excel(postcodes, base_output_folder):
    with pd.ExcelWriter(
        os.path.join(base_output_folder, "48_hour_forecast.xlsx"), engine="xlsxwriter"
    ) as writer:
            ...
            for entry in combined_data:
                ...
                row = {
                    ...
                    "Nuclear": next(
                        (
                            item["perc"]
                            for item in entry["generationmix"]
...
def generate_tiles(postcode, base_output_folder):
    ...
    for entry in combined_data:
        ...
        timestamp = datetime.strptime(entry["from"], "%Y-%m-%dT%H:%MZ").strftime(
            "%Y-%m-%d_%H-%M"
        )
